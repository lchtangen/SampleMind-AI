...
Activate for any task involving: dev environment setup, scripts/setup-dev.sh, scripts/start.sh,
GitHub Actions workflows, ci.yml, python-lint.yml, WSL2 configuration, git performance
(fsmonitor, untrackedcache), pre-commit hooks, pyproject.toml build system, uv sync,
pnpm install, environment variables, .env.example, first-time setup, "how do I set up this
project", "install dependencies", "configure CI", or "add a GitHub Action".

File patterns: `scripts/setup-dev.sh`, `scripts/start.sh`, `scripts/*.sh`,
`.github/workflows/*.yml`, `.github/workflows/python-lint.yml`, `pyproject.toml`,
`.pre-commit-config.yaml`, `.env.example`, `Makefile`.

Code patterns: `uv sync`, `astral-sh/setup-uv`, `setup-uv@v`, `runs-on: ubuntu`,
`runs-on: macos`, `apt-get install -y libwebkit2gtk`, `pre-commit install`,
`git config core.fsmonitor`, `#!/usr/bin/env bash`, `SAMPLEMIND_SECRET_KEY=`, `fail_under =`,
